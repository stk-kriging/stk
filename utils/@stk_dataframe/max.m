...
if (nargin == 3) && (dim ~= 1),
    errmsg = 'max() always acts columnwise on stk_dataframe objects.';
...
%!test warning off; z = max(df1, df3); warning on;
%!assert (~isa(z, 'stk_dataframe') && isequal(double(z), max(x1, x3)));
