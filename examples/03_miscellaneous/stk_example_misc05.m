% STK_EXAMPLE_MISC05  Estimations of parameters using various criteria
%
% This examples compares different criteria to estimate the
...
% List of criterion
list_crit = {@stk_param_relik, @stk_param_loomse, @stk_param_relik, @stk_param_loopvc};
...
stk_figure('stk_example_misc05 (a): Predictions');
for estim = 1:nb_crit
    ...
    is_loo   = ~isempty(strfind(name_crit, 'loo'));  % Is leave-one-out ?
    
    if strcmp(name_crit, 'stk_param_relik')
        if is_first_relik
            % If it is the first time we use relik, do nothing, just
            % indicate it.
