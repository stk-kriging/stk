...
elseif (diff >= 2) && (diff <= nb_params),
    ...
    if pairwise,
        ...
        k = (repmat (xs(:, ind), 1, ny) - repmat (ys(:, ind)', nx, 1)) .^ 2 ...
            .* Kx_cache;
    end
else
...
%! K2 = stk_gausscov_aniso (param, struct ('a', u), struct ('a', v), -1);
%! K3 = stk_gausscov_aniso (param, stk_dataframe (u), stk_dataframe (v), -1);
%! assert (isequal (K1, K2));
%! assert (isequal (K1, K3));
